...
import { Ticket, Mensagem } from "../../lib/api"
      ...
      {mensagens.map(msg => {
        ...
        return (
          <div key={msg.id} className={`flex gap-3 ${isSuporte ? "justify-end" : "justify-start"}`}>
            ...
            <div className={`max-w-[75%] p-4 rounded-2xl shadow-sm relative ${isSuporte ? "bg-blue-600 text-white rounded-tr-sm" : "bg-slate-800 border border-slate-700 text-slate-200 rounded-tl-sm"}`}>
